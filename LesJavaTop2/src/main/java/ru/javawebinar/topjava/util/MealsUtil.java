...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    public static List<MealWithExceed> hshs(Collection<Meal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
    	Map<LocalDate, Integer> caloriesSumByDate = meals.stream()
    			.collect(
    					Collectors.groupingBy(Meal::getDate, Collectors.summingInt(Meal::getCalories))
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
...
//    	return 
    			...
    			.filter(meal -> {
					boolean between = DateTimeUtil.isBetween(meal.getTime(), startTime, endTime);
					return between;
				}  )
    			.map(meal -> {
					LocalDate date = meal.getDate();
					
					Integer calorIn1Day = caloriesSumByDate.get(date );
					return createWithExceed(meal, calorIn1Day > caloriesPerDay);
				})
    			.collect(Collectors.toList());
				return collect;
    }
